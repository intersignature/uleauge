 ...
 * @author CPCust
 */
@WebServlet(urlPatterns = {"/ChgServlet"})
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            int team_id = Integer.parseInt(request.getParameter("team_id"));
             String new_cap = request.getParameter("new_cap");
           String username = request.getParameter("username");
           String chgcap = "Update db_accessadmin.Team  set Team_Cap = ? WHERE  Team_ID = ?";
            PreparedStatement update = conn.prepareStatement(chgcap);  
            update.setString(1, new_cap);  
            update.setInt(2, team_id);  
            update.execute();
            update.close();
            ...
            PreparedStatement update1 = conn.prepareStatement(memleave);  
            update1.setInt(1, team_id);  
            update1.execute();
            update1.close();
